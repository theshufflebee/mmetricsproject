...
#define market open times
market_open_hour <- 9
market_close_hour <- 16 #16th hour is closed

#adjust timestamp for non-market hours
social_hourly <- social_hourly %>%
  mutate(
    timestamp = with_tz(timestamp, tzone = tz),  # ensure correct tz
    hour = hour(timestamp),
    date = as.Date(timestamp),
    is_market_hour = hour >= 9 & hour < 16,
    adjusted_time = case_when(
      is_market_hour ~ floor_date(timestamp, unit = "hour"),
      TRUE ~ make_datetime(
        year = year(timestamp),
        month = month(timestamp),
        day = day(timestamp) + ifelse(hour >= 16, 1, 0),
        hour = 9, min = 0, sec = 0,
        tz = tz)))
