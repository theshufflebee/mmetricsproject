...
social_hourly$adjusted_time = as.POSIXct(social_hourly$timestamp,format = "%Y-%m-%d %H:%M:%S")
...
#find count
tweetcount_alltime = dplyr::select(social_hourly,timestamp,adjusted_time,N)
...
#find dummy
tweetdummy_alltime = dplyr::select(social_hourly,timestamp,adjusted_time,dummy)
...
#find count
tariff_alltime = dplyr::select(social_hourly,timestamp,adjusted_time,total_tariff)
...
#find count
trade_alltime = dplyr::select(social_hourly,timestamp,adjusted_time,total_trade)
...
#find count
china_alltime = dplyr::select(social_hourly,timestamp,adjusted_time,total_china)

#select time period
china = filter(china_alltime,
                  between(timestamp, 
                          as.Date('2018-01-01'), 
                          as.Date('2025-05-07')))
...
#find count
positive_alltime = dplyr::select(social_hourly,timestamp,adjusted_time,prop_positive)
...
#find count
negative_alltime = dplyr::select(social_hourly,timestamp,adjusted_time,prop_negative)
...
armax_data <- armax_data %>%
  left_join(select(tweetdummy, -timestamp), by = c("timestamp" = "adjusted_time")) 
armax_data <- armax_data %>%
  left_join(select(tweetcount, -timestamp), by = c("timestamp" = "adjusted_time")) 
armax_data <- armax_data %>%
  left_join(select(tariff, -timestamp), by = c("timestamp" = "adjusted_time")) 
armax_data <- armax_data %>%
  left_join(select(trade, -timestamp), by = c("timestamp" = "adjusted_time")) 
armax_data <- armax_data %>%
  left_join(select(china, -timestamp), by = c("timestamp" = "adjusted_time")) 
armax_data <- armax_data %>%
  left_join(select(positive, -timestamp), by = c("timestamp" = "adjusted_time")) 
armax_data <- armax_data %>%
  left_join(select(negative, -timestamp), by = c("timestamp" = "adjusted_time")) 
...
res3 = auto.armax.r(armax_data$SPY_vol, x=armax_data$total_tariff, 
                max_p = 7, max_q = 7, max_r = 7, criterion = "AIC", latex=F)
