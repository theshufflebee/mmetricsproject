...
res.old = auto.armax(armax_data$SPY_vol,xreg=armax_data$total_tariff,nb.lags=2,
                latex=F,max.p = 7, max.q = 7, max.d=0)

res.custom = armax(armax_data$SPY_vol, xreg=armax_data$total_tariff, nb.lags=2,
                   p=5, q=0, d=0, latex=F)

res.armax = auto.armax.r(armax_data$SPY_vol, x=armax_data$total_tariff, 
...
phi = res.old$model$phi
theta = res.old$model$theta
sigma = sqrt(res.old$sigma2)
coefficients = coef(res.old)
beta = coefficients[grep("^total_tariff", names(coefficients))]

IRFdata1 <- sim.arma(c=0,phi=phi,theta=beta,sigma=sigma,
...
IRFdata2 <- sim.arma(c=0,phi=phi,theta=beta,sigma=sigma,
                 T=nb.periods,y.0=rep(0,length(phi)),
                 nb.sim=1,make.IRF=1)
...
IRFdata3 <- sim.arma(c=0,phi=phi,theta=beta,sigma=sigma,
                 T=nb.periods,y.0=rep(0,length(phi)),
                 nb.sim=1,make.IRF=1)


IRF = cbind.data.frame(1:nb.periods,IRFdata1,IRFdata2,IRFdata3)
colnames(IRF) = c("Period","IRF","IRF2","IRF3")

ggplot(IRF,aes(x = Period, y = IRFdata1)) +
    geom_line(color = "steelblue", size = 1.2) +
    geom_point(color = "red", size = 2) +
...
ggplot(IRF,aes(x = Period, y = IRFdata2)) +
    geom_line(color = "steelblue", size = 1.2) +
    geom_point(color = "red", size = 2) +
    labs(title = "Custom ArmaX IRF") +
    theme_minimal()

ggplot(IRF,aes(x = Period, y = IRFdata3)) +
    geom_line(color = "steelblue", size = 1.2) +
    geom_point(color = "red", size = 2) +
    labs(title = "Select AIC ARMAX IRF") +
    theme_minimal()


irf.plot(res.old,nb.periods)
irf.plot(res.custom,nb.periods)
irf.plot(res.armax$model,nb.periods)

```


# Time Series Simulations
```{r sim armax, warning=F, message=F}
nb.periods = length(SPY_volatility$r_vol_h)
...
ggplot(IRF,aes(x = Period, y = IRFdata1)) +
    ...
    labs(title = "AutoArmaX") +
    theme_minimal()

ggplot(IRF,aes(x = Period, y = IRFdata2)) +
    geom_line(color = "steelblue", size = .2) +
    labs(title = "Custom ArmaX") +
    theme_minimal()
