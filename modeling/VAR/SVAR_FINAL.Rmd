...
the greater the persistence of a shock - up to unrealistic levels, i.e., 
150 days for Tweet Count, which is implausible. Therefore, we chose to fix the
...
to compute a robust standard errors.
...
main variables. For all estimations, using the US ETF market, the significant 
coefficients of post-related variables were consistently negative, while the 
...
When analyzing the European and Chinese ETF markets, we observe similar patterns 
though with lower magnitude -except for the impact of Tariff and China on the 
ASHR market, where the cumulative effects show no positive impact. Additionally, 
the European ETF market appears to react more strongly than the ASHR market to 
...
the S&P market. Surprisingly, a large number of Granger test in the chinese and 
European markets indicate strong Granger causality, which may point to a limitation 
...
For the splitting framework, the results were similar and striking [*IRFchinasecondtermVAR*]{style="color:red"}. While we
observed relatively small shock effects and almost entirely negatives coefficients 
during the first term ([*descFirsttermfigVAR*]{style="color:red"}), leading the 
cumulative IRFs to indicate a negative impact of posts, the shock effects in the
...
statistically significant, but the cumulative IRFs clearly show an high positive 
impact on except for TweetDummy and TweetCount, whose
coefficients and cumulative IRFs displayed similar patterns to those observed 
...
Regarding the ASHR market, we found results similar to those for the US ETF
market. Surprisingly, in the case of the European market, we observed a positive
