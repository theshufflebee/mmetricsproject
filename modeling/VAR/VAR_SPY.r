...
##with N
y2 =cbind(Vdata$r_vol_h, Vdata$N)

y_lag2 = VARselect(y2, lag.max = 80)
y_list2 = list(y_lag2)


##AIC
resultats21 <- lapply(y_list2, function(x) x$criteria["AIC(n)", ])

resultats21 = as.data.frame(resultats21)

resultats21 = resultats21 %>%
  rename(name = 1) %>%
  mutate(
    n = c(1:length(name))
  )

ggplot (resultats21, aes(x=n, y= name)) +
...
##HQ
resultats22 <- lapply(y_list2, function(x) x$criteria["HQ(n)", ])

resultats22 = as.data.frame(resultats22)

resultats22 = resultats22 %>%
  rename(name = 1) %>%
  mutate(
    n = c(1:length(name))
  )

ggplot (resultats22, aes(x=n, y= name)) +
  geom_line(color = "steelblue") +
  labs(title = "hourly Returns HQ",x="n" , y = "HQ") +
  theme_minimal()



##SC
resultats32 <- lapply(y_list2, function(x) x$criteria["SC(n)", ])

resultats32 = as.data.frame(resultats32)

resultats32 = resultats32 %>%
  rename(name = 1) %>%
  mutate(
    n = c(1:length(name))
  )

ggplot (resultats32, aes(x=n, y= name)) +
  geom_line(color = "steelblue") +
  labs(title = "hourly Returns SC",x="n" , y = "SC") +
  theme_minimal()


##FPE
resultats42 <- lapply(y_list2, function(x) x$criteria["FPE(n)", ])

resultats42 = as.data.frame(resultats42)

resultats42 = resultats42 %>%
  rename(name = 1) %>%
  mutate(
    n = c(1:length(name))
  )

ggplot (resultats42, aes(x=n, y= name)) +
  geom_line(color = "steelblue") +
  labs(title = "hourly Returns FPE",x="n" , y = "FPE") +
  theme_minimal()

#-----------------------------------------------------------------------------------------------
##let's test for  17 lag (cannot do more, because from 17 lag there is eigen higher outside unit circle)

#dummy
est.VAR <- VAR(y,p=16)

#N
est.VARN = VAR(y2,p=16)
summary(est.VARN)
...
###y1 is volatility and y2 is tweet 

## Check stationarity of the estimated model:
PHI <- make.PHI(Phi)
print(abs(eigen(PHI)$values))

###16 est le maximum avant qu'une eigenvalue ne devienne >1 
...
##create SVAR model
###here we assume that a shock of tweet (N or dummy) impacts volatility, but volatility 
###shock doesn't impact tweet
...
plot(vd)
#----------------------------------------------------------------------------------------------
###now we make the same assumption as precendently, but here
###we assume that volatility (not shock) has no impact on tweet
#a chier (enfin ça va)
amat = matrix(c(NA,0, NA, NA), 2)
bmat2 = matrix(c(1, 0, NA, 1), 2)

SVAR3 = SVAR(est.VARN, estmethod = c("scoring", "direct"), Amat = NULL, Bmat = bmat)
summary(SVAR3)

IRF3 <- irf(SVAR3, response = "y1", impulse = "y2", 
           n.ahead = 40, ortho = TRUE, boot = TRUE)
par(mfrow = c(1, 1), mar = c(2.2, 2.2, 1, 1), cex = 0.6)
plot(IRF)
...
#does dum granger cause vol
grangertest(y[,c("N", "vol")], order = 16)


#Variane decomposition
vd3 = fevd(SVAR3, n.ahead = 20)
plot(vd3)
#------------------------------------------------------------------------------------------------
...
y_lag3 = VARselect(y3, lag.max = 80)
y_list3 = list(y_lag3)
...
grangertest(y3[,c("vol","tarrif")], order = 16)
#mdr
...
#does vol granger cause tarrif
grangertest(y4[,c("vol","trade")], order = 16)
#mdr 
...
grangertest(y5[,c("vol","anger")], order = 16)


#does anger granger cause vol
grangertest(y5[,c("anger", "vol")], order = 16)
...
grangertest(y6[,c("vol","fear")], order = 16)
 

#does fear granger cause vol
grangertest(y6[,c("fear", "vol")], order = 16)
...
grangertest(y7[,c("vol","anticipation")], order = 16)
...
grangertest(y9[,c("vol","disgust")], order = 16)
 

#does disgust granger cause vol
grangertest(y9[,c("disgust", "vol")], order = 16)
...
grangertest(y10[,c("vol","joy")], order = 16)
...
grangertest(y11[,c("vol","sadness")], order = 16)
 

#does sadness granger cause vol
grangertest(y11[,c("sadness", "vol")], order = 16)
...
grangertest(y12[,c("vol","surprise")], order = 16)
 

#does surprise granger cause vol
grangertest(y12[,c("surprise", "vol")], order = 16)
...
grangertest(y13[,c("vol","trust")], order = 16)
...
grangertest(y14[,c("vol","negative")], order = 16)
...
grangertest(y15[,c("vol","positive")], order = 16)
...
grangertest(y16[,c("vol","total_posneg")], order = 16)
 

#does total_posneg granger cause vol
grangertest(y16[,c("total_posneg", "vol")], order = 16)
...
grangertest(y17[,c("vol","total_sentiment")], order = 16)
#mdr 

#does total_sentiment granger cause vol
grangertest(y17[,c("total_sentiment", "vol")], order = 16)
...
grangertest(y18[,c("vol","prop_anger")], order = 16)


#does prop_anger granger cause vol
grangertest(y18[,c("prop_anger", "vol")], order = 16)
...
grangertest(y19[,c("vol","prop_fear")], order = 16)


#does prop_fear granger cause vol
grangertest(y19[,c("prop_fear", "vol")], order = 16)
...
grangertest(y20[,c("vol","prop_anticipation")], order = 16)


#does prop_anticipation granger cause vol
grangertest(y20[,c("prop_anticipation", "vol")], order = 16)
...
grangertest(y21[,c("vol","prop_disgust")], order = 16)


#does prop_disgust granger cause vol
grangertest(y21[,c("prop_disgust", "vol")], order = 16)
...
grangertest(y27[,c("vol","prop_joy")], order = 16)


#does prop_joy granger cause vol
grangertest(y27[,c("prop_joy", "vol")], order = 16)
...
grangertest(y22[,c("vol","prop_sadness")], order = 16)


#does prop_sadness granger cause vol
grangertest(y22[,c("prop_sadness", "vol")], order = 16)
...
grangertest(y23[,c("vol","prop_surprise")], order = 16)


#does prop_surprise granger cause vol
grangertest(y23[,c("prop_surprise", "vol")], order = 16)
...
grangertest(y24[,c("vol","prop_trust")], order = 16)


#does prop_trust granger cause vol
grangertest(y24[,c("prop_trust", "vol")], order = 16)
...
grangertest(y25[,c("vol","prop_negative")], order = 16)


#does prop_negative granger cause vol
grangertest(y25[,c("prop_negative", "vol")], order = 16)
...
grangertest(y26[,c("vol","prop_positive")], order = 16)


#does prop_positive granger cause vol
grangertest(y26[,c("prop_positive", "vol")], order = 16)
