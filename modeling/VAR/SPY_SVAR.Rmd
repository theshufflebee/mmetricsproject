...
```{r Estime china}
y5 = cbind(Vdata$china , Vdata$SPY_vol)
colnames(y5)[1:2] <- c("china", "vol")
est.VAR5 <- VAR(y5,p=6)
mod_vol5 <- est.VAR5$varresult$vol
screenreg(mod_vol5, digits = 6)
```
...
```{r estime Interaction, warning=FALSE, message=FALSE}
...
Here we look for the first and second mandate effect of posts. We will use the dummy variable as a proxy for the posts
...
```{r estime first mandate, warning=FALSE, message=FALSE}
...
screenreg(mod_vol_f_d, digits = 6)
```


```{r B mat first mandate}
...
#irf creation
irf_res_f_d <- irf(est.VAR_f_d, impulse = "dummy", response = "vol", 
...
single_varirf_f_d %>% 
  ...
  geom_line() +
  theme_light() +
  ggtitle("IRF First Mandate Dummy on Volatility")+
  ylab("")+
  xlab("") +
  theme_minimal()
...
```{r estime second mandate, warning=FALSE, message=FALSE}
...
screenreg(mod_vol_s_d, digits = 6)
```
...
#irf creation
irf_res_s_d <- irf(est.VAR_s_d, impulse = "dummy", response = "vol", 
...
single_varirf_s_d %>% 
  ...
  geom_line() +
  theme_light() +
  ggtitle("IRF Second Mandate Dummy on Volatility")+
  ylab("")+
  xlab("") +
  theme_minimal()
