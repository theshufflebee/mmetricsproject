...
and the mentions of words $Tariff$, $Trade$, and $China$. We first note on the table
 in section \@ref(tab:spy-table) that all the x-regressors are significant,
...
in Section \@ref(tab:means-table). We in fact predict that an 
...
impulse response function (IRF) for this shock, in \@ref(fig:SPY-IRF) Notice that
...
\@ref(tab:spy-table-terms), the first interesting result
...
now than before. We've found similar IRF's as for the full timeframe. Tables \@ref(fig:SPY-IRF)
...
In Section \@ref(tab:SPY-res-test), the pvalues being zero
