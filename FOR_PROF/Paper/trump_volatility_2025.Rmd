...
header-includes:
  ...
  - \usepackage{placeins}
  - \usepackage{float}
geometry: "left=1.6cm,right=1.6cm,top=1cm,bottom=2cm"
...
social media posts has drastically increased.
...
```{r fig1, eval=FALSE, echo=FALSE, fig.cap="Terminally Online: Trump's Twitter & Truth Social Posts (EDT)", warning=FALSE, cache=TRUE}
...
We note that the last few months show a new era of never seen before
levels of volatility. Shocks on volatility recently have reached, and even surpassed
levels seen during the COVID-19 pandemic.
...
[^1]: For instance, if Trump tweets on Good Friday (market holiday), then the 
...
decline over time. Something to note is that in our analyses of IRF, when including
...
now than before. We've found similar IRF as for the full timeframe. 
Finally, we can check the residuals of all these models to test them somewhat. 
We find that p-values are zero for the full timeframe and first term models, which 
indicates that there is autocorrelation in the residuals, thus suggesting that these 
estimations are problematic. However, for the second term, the p-values are quite high 
(~ 0.8 for $Tariff$), lending support to our models on the split sample. 
These results suggest that perhaps ARMA-X models are
...
```{r armaxirf1, results="asis", warning=F, message=F, echo=FALSE, cache=TRUE, fig.cap="ARMA-X IRF", fig.width=6, fig.height=2.5}
...
```{r armaxirf2, warning=F, message=F, echo=FALSE, cache=TRUE, eval=FALSE}
...
### SPY VAR Models (Jan 2014 - May 2025) 
...
```{r estime China, message=FALSE, warning=FALSE, echo=FALSE, cache=TRUE, results="asis"}
...
```{r vartable1, message=FALSE, warning=FALSE, results='asis', echo=FALSE, cache=TRUE}
...
{\\vspace{2pt}Each VAR regression has only two variables: AHV and X. The column names represent the X variable for the selected model. \\\\ %stars.}")
...
```{r VARirf1, warning=FALSE, message=FALSE, echo=FALSE, cache=TRUE, fig.cap="VAR IRF 1", fig.width=6, fig.height=2.3}
nb.sim = 7*7
...
```{r VARirf2, warning=FALSE, message=FALSE, echo=FALSE, cache=TRUE, fig.cap="VAR IRF 2", fig.width=6, fig.height=2.5, results='asis', fig.pos="H"}
ggplot(Yd3,aes(x=period, y=cumsum(response))) +
  geom_hline(yintercept = 0, color="red") +
  geom_line() +
  theme_light() +
  ggtitle("Cumulative Tariff Mention Shock: Jan 2014 - May 2025") +
  ylab("")+
  xlab("Hours after shock") +
  theme_minimal() +
            theme(axis.text.x = element_text(angle = 45, hjust = 1))
...
### SPY VAR First-Term Models

```{r datasetup2, results=FALSE, warning=FALSE, message=FALSE, echo=FALSE, cache=TRUE}
...
```{r first mandate dum, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r first mandate N, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r first mandate tariff, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r first mandate trade, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r first mandate china, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r table first term, message=FALSE, warning=FALSE, results='asis', echo=FALSE, cache=TRUE, eval=TRUE}
...
### SPY VAR Second-Term Models

```{r second mandate dum, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r second mandate N, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r second mandate tariff, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r second mandate trade, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r second mandate china, warning=FALSE, message=FALSE, results="asis", echo=FALSE, cache=TRUE}
...
```{r B mat second mandate china, warning=FALSE, message=FALSE, echo=FALSE, cache=TRUE}
...
```{r Table second Term, message=FALSE, warning=FALSE, results='asis', echo=FALSE, cache=TRUE}
