...
csl: apa.csl
abstract: "In this short paper, we aim to asses to what extent financial markets may react to Donald Trump’s social media posts, and more specifically, the effect on average realised volatility. We do so using both ARMA-X and SVAR models, with data spanning the 1st of January 2014, to the 7th of May 2025, over various time horizons and independent variables. Being limited by persistent auto-correlation in the residuals, we find limited evidence that there is a statistically significant positive effect, and provide some explanations as to why this might be the case."
---
\newpage
...
```{r fig1, echo=FALSE, fig.cap="Number of Twitter & Truth Social Posts (EDT Timezone)", warning=FALSE, fig.width=6, fig.height=4}
 #Create the scatter plot
ggplot(truths, aes(x = day, y = time_shifted)) +
  geom_point(alpha = 0.5, color = "blue", size = 0.45) +  # Transparancy to create "heatmap"
  scale_y_continuous(
...
Consequently, a large part of the literature has focused on the relation
...
we had to take proxies under the form of ETFs that track them. Our 3 markets of
...
through a free stock API, Alpha Vantage. Our timeframe starts on the first 
...
but closes at 4:00 pm. Plotting this data, we observe that the last few months display 
...
seen as Trump was then banned. Due to that, we have a gap in data going until February 2022, 
...
then construct multiple variables this dataset. These include
a dummy for whether there was a post ($TweetDummy$), the number of posts in an hour 
($TweetCount$), and counts
for mentions of certain words ($Tariff$,$Trade$, \& $China$). Furthermore, we applied some simple 
...
which sees minimal AIC loss and allows the simplifying  of our models considerably.
...
decline over time. Something to note is that in our various analyses, when including
...
suggests that there is significant auto-correlation, thus suggesting that these 
...
not quite right in this context as it is not unreasonable to think that Trump does 
in fact react to market movements, which would break the exogeneity assumption
that is critical for this type of model. With this information, we decide to run
an SVAR model to account for possible endogeneity.
...
hours ($TweetDummy$ and $Count$). You can find the IRFs for $Tariff$ on Figures 
...
where we notice the results are similar and striking. While we
