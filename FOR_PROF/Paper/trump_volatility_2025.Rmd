...
```{r fig1, eval=FALSE, echo=FALSE, fig.cap="Terminally Online: Trump's Twitter & Truth Social Posts (EDT)", warning=FALSE}
...
AR terms. 
Note that we ran all these models on the VGK and ASHR ETFs as well, though no
...
main variables. Table \@ref(tab:VAR) shows all estimations using the SPY ETF, where
...
For the $Tariff$, $Trade$ and $China$ variables, the first, second and sometimes forth 
...
the IRFs experienced a positive shock and remain elevated $Tariff$, $Trade$ and $China$), 
or a highly positive shock occurs, but the cumulative effect turns negative after a few 
hours ($TweetDummy$ and $Count$). You can find the IRFs for $Tariff$ on Figure 
\@ref(fig:VARirf1).
...
When analyzing the European and Chinese ETFs, we observe similar patterns 
...
in the case of the European market, we observe a positive
...
```{r VARirf1, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="VAR IRF 1", fig.width=6, fig.height=2.3}
nb.sim = 7*7
...
```{r VARirf2, warning=FALSE, message=FALSE, echo=FALSE, fig.cap="VAR IRF 2", fig.width=6, fig.height=2.5, results='asis', fig.pos="H"}
